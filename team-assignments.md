...
- [x] **Energy Cost**（2026-09-05 重測）：Pixel 7 螢幕關閉、Pixel 8a 當鄰居，baseline 與 Emergency Mode 交錯 6 輪 × 60 筆 ⇒ **385 mW → 439 mW，+54 mW（+14%）**，約每小時多耗 0.3% 電量。原始 CSV 在 `experiments/results/energy-raw/pixel7-{baseline,emergency-mode}-2026-09-05.csv`，已進 `experiments/results/report.md` 第 5 節（透過 `simulator/lib/report.mjs` 生成，非手改）。
  **同日稍早那組 22.35 → 26.78 mW 已作廢**：手機插著 USB 且電量全滿，電池電流在零附近震盪，量到的是計量器雜訊——兩個「不同條件」的 min／p25／median 完全相同（1.35／10.83／20.31）即為證據。詳見 `experiments/results/energy-raw/README.md`。
