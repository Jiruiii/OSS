...
>
> 狀態：2026-09-24 規劃；**2026-09-26 實作完成（實機驗證除外）**。任務總覽與進度追蹤見 `docs/mvp-remaining-tasks.md` F 段。

## 實作記錄與偏離（2026-09-26）

- **表單契約以已合併的 Flutter UI 為準**（`2026-09-24-flutter-crowd-alert-evacuation-ui.md`）：類別是 `ROAD_BLOCKAGE`／`FLOOD`／`FIRE_SMOKE`／`TRAPPED_INJURED`／`OTHER`，沒有嚴重度欄位（改由類別對應 severity），描述上限 160 字，成功回傳 `{event_id, apply_state: UNVERIFIED, delivery_state: PENDING}`，錯誤碼 `invalid_input`（不是 `invalid_arguments`）。Task 4 的表單、入口與確認步驟已在該計畫完成。
- **Task 1 Step 1 第五個情況改為「`device:` 金鑰缺少 `signer_public_key` 在 schema 階段失敗」**，不是「所有 `crowd.*` 都必須帶公鑰」。原因：`data/fixtures/neihu` 的 replay fixture 與 Android `signed-events.json` 都有以內建 demo 金鑰簽的 `crowd.*` 事件；依金鑰種類觸發規則，就不必改寫這些 fixture，而且 `official.*` 的信任規則一樣不變（有反向測試）。
- **座標範圍改為全台含離島**（經度 116.5–122.5、緯度 20.5–26.8），因為 App 已擴大為全台地圖。
- **單一 chunk 上限 4 KB**，不是 2 KB：最長的合法回報（160 個中文字加地址提示）約 2.4 KB。
- **`provenance` 不帶 `node_id`**，避免把節點 id 和裝置金鑰綁在一起。
- **Task 3 的 Flutter 顯示**：`MeshEvent.isCurrentAt()` 會把 `UNVERIFIED` 當成不可顯示，原本送出的回報永遠不會出現在地圖上；新增 `isShownAt()` 讓有效的 `UNVERIFIED` 事件以紫色顯示。
- **Task 6 Step 3／Step 4 的上下行**：debug-only `CrowdDebugReceiver`（`adb shell am broadcast`）負責匯出回報與匯入官方 chunk；`attest` 輸出的 event batch 可直接交給 `build`。
- **Task 8（選做）已完成**：`flutter/lib/data/corroboration.dart`。
- **未做**：Task 4 Step 4、Task 5 Step 5、Task 7 Step 3 的實機驗證；`./gradlew connectedDebugAndroidTest`（沒有連接裝置，instrumented 測試只確認可編譯）。
...
- [x] **Step 1: 寫失敗測試。** 測試以下五種情況： **（2026-09-26：第五種情況改為「device: 金鑰缺 signer_public_key」，理由見上方實作記錄）**
...
- [ ] **Step 2: 執行 `npm test`，確認新測試失敗、舊測試全過。** **（2026-09-26：未做：測試與實作同時撰寫，沒有先跑一次確認失敗）**
- [x] **Step 3: 實作。** `verifyEvent` 的 trust 階段改為兩條路徑：
...
- [x] **Step 4: 用新的 `pipeline/tools` 腳本產生 `fixtures/crowd-reports-v0.json`**。用固定種子產生測試金鑰，這把私鑰只能用在 fixture。重跑 `npm test` 必須全過。
- [x] **Step 5: 更新 `docs/data-contract-v0.md`。** 寫清楚裝置簽章證明什麼（內容沒被竄改、出自同一台裝置）、不證明什麼（內容屬實）。
...
- [x] **Step 1: 寫失敗測試。** 用 Task 1 的 fixture 跑同樣五種情況；另外做一次往返測試：`DeviceSigningKey` 簽出來的事件要能通過 `EventVerifier`。
- [ ] **Step 2: 執行 `./gradlew testDebugUnitTest`，確認新測試失敗。** **（2026-09-26：未做：同上）**
- [x] **Step 3: 實作 `EventVerifier` 的兩條 trust 路徑。** 規則和 JS 版逐字對應，註解標明對應 `contract.mjs` 的哪一段。
- [x] **Step 4: 實作 `DeviceSigningKey`。**
...
- [x] **Step 1: 寫 `CrowdReportFactoryTest`。** 要檢查的項目：
  ...
  - 備註欄超過上限會被拒絕。
  - 座標超出內湖 bounds 會被拒絕。
- [x] **Step 2: 實作 factory 與 `MeshRepository.createCrowdReport`。** 必須走 `EventIngestor.ingest`，不能直接寫 DAO，確保本機回報和收到的回報走完全相同的驗證。
- [x] **Step 3: bridge method。** 參數驗證失敗時回傳 `invalid_arguments`，並寫 `MapBridgeProtocol` 單元測試。 **（2026-09-26：錯誤碼依 UI 契約用 invalid_input）**
- [ ] **Step 4: instrumented 測試。** 回報後 `observeEvents()` 要出現這筆事件，且 `apply_state = UNVERIFIED`。 **（2026-09-26：測試已寫好並可編譯，沒有連接裝置，尚未執行）**
- [x] **Step 5: 更新 `FlutterMapBridge` 類別註解。** 說明為什麼新增寫入 method 仍然符合「Flutter 沒有 Room 寫入路徑」：Flutter 只提供欄位，驗證和寫入都在 Android。
...
- [x] **Step 1: 寫 widget 測試。** 包含：表單必填驗證；送出時用正確參數呼叫 bridge（mock `MethodChannel`）；送出前有確認步驟；bridge 回錯時顯示錯誤，不假裝成功。 **（2026-09-26：由 UI 計畫完成）**
- [x] **Step 2: 實作表單。** 類型、嚴重度、備註（上限 200 字，並提醒不要填個資）、位置。位置預設用長按點或 GPS，可以微調。 **（2026-09-26：由 UI 計畫完成：沒有嚴重度欄位，描述上限 160 字）**
- [x] **Step 3: `flutter test` 全過。** **（2026-09-26：唯一失敗是既有的 PMTiles SHA 檢查，與本功能無關）**
- [ ] **Step 4: 實機手動驗證。** 關閉網路後回報，強制結束 App 再重開，回報仍然在。 **（2026-09-26：未做：需要實機）**
...
- [x] **Step 1: 決定 chunk 包裝方式並寫進 `docs/peer-sync-v0.md`。** 建議做法：
...
- [x] **Step 2: 寫失敗測試。** 包含：往返編碼後驗證通過；中繼節點竄改後被拒；crowd chunk 的 `signing_key_id` 與內含事件的 `signing_key_id` 不同時被拒；`computeDiff` 對 crowd dataset 能正確算出缺少的回報。
- [x] **Step 3: 實作。** 另外加兩項防濫用機制： **（2026-09-26：單一 chunk 上限改為 4 KB）**
  - 單一 chunk 上限 2 KB。
  - 每個節點最多保存 200 筆他人回報，超過時先淘汰已過期的，再淘汰最舊的。
- [x] **Step 4: JVM 測試、`npm test`、`npm run sim:check` 全過。**
- [ ] **Step 5: 實機驗證（三機）。** A 回報 → B 轉傳 → C 收到。直接讀 C 的 Room，確認事件存在、狀態為 `UNVERIFIED`、`signing_key_id` 是 A 的。結果記錄到 `docs/mvp-remaining-tasks.md` F 段。 **（2026-09-26：未做：需要實機；JVM 測試已模擬 A→B→C）**
...
- [x] **Step 2: 實作。** 確認事件的 `expires_at` 預設等於原回報的 `expires_at`。
- [x] **Step 3: 上行（demo 版）。** 新增 debug-only 的 Android 匯出，把本機收到的 crowd 回報寫成 JSON 檔（`adb pull` 取出），再交給 `attest`。真的上傳 API 列為後續工作，不在本計畫範圍。 **（2026-09-26：debug-only CrowdDebugReceiver）**
- [x] **Step 4: 確認事件用既有的 `build` 指令打包成官方分片**，走原本的下行路徑。
...
- [x] **Step 2: 實作。** 這是純 Dart 的顯示邏輯，Android 的 `ApplyState` 不需要修改。
- [ ] **Step 3: 實機 demo。** A 回報 → mesh → 匯出 → 筆電 `attest` → `build` → 下行分片 → A、B、C 都顯示「已查證」。更新 `experiments/demo.md`。 **（2026-09-26：未做：需要實機）**
...
- [x] **Step 1: 定義規則。** 同類型、距離 150 m 內、2 小時內，來自不同 `signing_key_id` 的回報數 ≥ 2，就顯示「N 人回報」。
- [x] **Step 2: 測試。** 同一金鑰重複回報不重複計數；已過期的回報不計；文字不得出現「驗證」「查證」字樣。
- [x] **Step 3: 在 `experiments/limitations.md` 寫明弱點。** 一個人用多支手機就能灌票，這不是驗證。

### Task 9: 文件與收尾

- [x] README：「核心功能」新增民眾回報；更新「限制」段落；「未來工作」移除已完成的部分。
- [x] `docs/mvp-remaining-tasks.md` F 段：勾選已完成項目。
- [x] `experiments/limitations.md`：記錄上行仍然是 demo 版檔案匯出、裝置金鑰可以被串連追蹤的隱私問題，以及沒有撤銷機制。
- [ ] 全套驗證：`npm test`、`python -m unittest discover -s tests`、`npm run sim:check`、`./gradlew testDebugUnitTest`、`./gradlew connectedDebugAndroidTest`（實機）、`flutter test`。 **（2026-09-26：未完成：沒有裝置可跑 connectedDebugAndroidTest；python 測試有一個既有失敗（test_android_splash））**
