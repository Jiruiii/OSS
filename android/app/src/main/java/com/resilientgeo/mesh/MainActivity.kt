 ...
 * team-assignments.md ("使用者手動開啟有明顯狀態提示"). Flipping it on/off
 * starts/stops [EmergencyModeService] (甲's minimal foreground-service stub,
 * proven to survive background/lock-screen on a real device — see that
 * class's doc comment) — no BLE behavior is wired to it yet beyond staying
 * alive.
 *
...
class MainActivity : AppCompatActivity() {
    ...
    ) { /* EmergencyModeService starts either way when toggled on — see its own doc comment. */ }
            ...
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                launch {
                    viewModel.emergencyModeEnabled.collect { enabled ->
                        ...
                        val serviceIntent = Intent(this@MainActivity, EmergencyModeService::class.java)
                        if (enabled) {
