...
object LayerChunkVerifier {
    ...
    fun verify(chunk: JSONObject, trustStore: TrustedKeyStore): LayerVerificationResult {
        ...
        // Shallow copy without the signature instead of re-parsing the whole
        // chunk from text (~1-2 s for the nationwide shelter layer on a phone).
        val signatureInput = JSONObject(chunk, chunk.keys().asSequence().filter { it != "signature" }.toList().toTypedArray())
    ...
    private fun validateShape(chunk: JSONObject): List<String> {
        ...
        if (!SHA256_RE.matches(chunk.optString("chunk_hash", ""))) errors += "chunk_hash is invalid"
        if (!SHA256_RE.matches(chunk.optString("manifest_hash", ""))) errors += "manifest_hash is invalid"
